...
//
float Yolov6EM::EM_BOX_THRESH = 0.4;
float Yolov6EM::EM_NMS_THRESH = 0.45;
...
//        spdlog::info("RGA_VERSION :{} ",ver);
        spdlog::debug("resize and padding with RGA!");
...
void Yolov6EM::Postpocess(float *prob, std::vector<Object> &objects, float scale) {

    decode_outputs((float *)prob, output_size, objects, scale, ori_width, ori_height,anchor_points,stride_tensor);
}
void Yolov6EM::draw_objects(const cv::Mat& bgr, const std::vector<Object>& objects)
{
...
void Yolov6EM::decode_outputs(float * prob, int output_size, std::vector<Object>& objects, float scale, const int img_w, const int img_h, const std::vector<AnchorPoints> &anchor_points, const std::vector<float> &stride_tensor){
    std::vector<Object> proposals;
    generate_yolo_proposals(prob, output_size, EM_BOX_THRESH, proposals,anchor_points,stride_tensor);
//    spdlog::info("  num of boxes before nms: {}",proposals.size());
    qsort_descent_inplace(proposals);

    std::vector<int> picked;
    nms_sorted_bboxes(proposals, picked, EM_NMS_THRESH);


    int count = picked.size();
    spdlog::debug("----yolov6EM   num of boxes: {}",count);
